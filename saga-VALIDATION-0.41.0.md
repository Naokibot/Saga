...
- Python<->Go differential conformance: **48/48 PASS** on the documentation-frozen source candidate.
- Module conformance: **14/14 PASS** on the documentation-frozen source candidate.

## Existing-language/runtime regression

Completed representative groups after documentation freeze:
...
Source-bound qualifications are regenerated again after this report is frozen and the final source manifest is written; the machine-readable validation JSON is the source of truth for the final manifest binding.
...
ArUco image recognition and video decode are real executed media paths and are distinct from these unexecuted items.
