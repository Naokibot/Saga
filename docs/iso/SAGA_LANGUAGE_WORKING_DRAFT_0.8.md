# Saga programming language — Working Draft 0.8

**Document status:** International Standard candidate working draft.  
**Document identifier:** SAGA-WD-0.8-2026-08-07  
**Normative language version:** 0.8  
...
An extension shall not silently change the meaning of a conforming Saga 0.8 program. Extension syntax shall be rejected unless explicitly enabled outside the Standard Core profile.
...
The normative grammar is `spec/saga-0.8.ebnf`. When the prose and grammar appear to conflict, the prose requirements and published defect resolutions take precedence until the next corrected draft.
...
Generic parameters are invariant in Saga 0.8. Specialization shall not change observable Standard Core behavior.
...
The normative EBNF is distributed as `spec/saga-0.8.ebnf` and its SHA-256 is recorded in the release checksum manifest.
