# Saga programming language — Working Draft 0.7

**Document status:** International Standard candidate working draft.  
**Document identifier:** SAGA-WD-0.7-2026-08-07  
**Normative language version:** 0.7  
**Reference implementation version:** 0.8.0  
...
7. implementation limits, diagnostic categories, conformance profiles, and command-line exit status requirements.
...
A program that satisfies all static requirements of its declared profile and does not exceed documented implementation limits.
...
- numeric limits and decimal context limits;
- resource ceilings;
...
An extension shall not silently change the meaning of a conforming Saga 0.7 program. Extension syntax shall be rejected unless explicitly enabled outside the Standard Core profile.

## 5 Source representation

### 5.1 Encoding

Source units shall be UTF-8. A single leading U+FEFF byte-order mark may appear and shall be ignored. Any other malformed UTF-8 sequence shall produce `SAGA-P001` or an equivalent input diagnostic before execution.
...
Integer literals are base-10 arbitrary-precision integers. Decimal literals denote exact base-10 decimal values. Numeric separators `_` may occur only between digits. Strings may use single or double quotes and shall decode specified escapes deterministically.

## 6 Lexical and syntactic processing

The normative grammar is `spec/saga-0.7.ebnf`. When the prose and grammar appear to conflict, the prose requirements and published defect resolutions take precedence until the next corrected draft.

An implementation shall diagnose malformed tokens before type checking. It shall not expose a host-language stack trace for a source nesting or source-size limit; it shall produce a controlled resource diagnostic.

The reference minimum ceilings are:

- source unit: 8 MiB;
- tokens per source unit: 1,000,000;
- syntactic nesting: 512;
- AST nodes per compilation: 1,000,000;
- source units per compilation: 1,024;
- source-unit dependency depth: 128.

An implementation may support larger values but shall not support smaller values when claiming Standard Core.
...
Dependencies are processed before the including unit. Each canonical source path is included once. In version 0.7, included declarations share one compilation namespace; source inclusion does not create a namespace alias.
...
The entry and test paths shall remain within the project root. `project.name` shall be 1–64 ASCII characters, start with an ASCII letter, and otherwise contain only ASCII letters, digits, `_`, or `-`; this prevents a project name from being interpreted as a path.
...
Decimal literals and decimal operations use base-10 arithmetic. The active precision is 16 through 10,000 significant digits in the reference profile. Rounding is round-half-even unless a future standard explicitly introduces another context.
...
Generic parameters are invariant in Saga 0.7. Specialization shall not change observable Standard Core behavior.
...
`private` members are accessible only through the declaring class's implementation. Reflection and serialization shall not expose private members. Constructor field arguments are evaluated left to right.
...
All task creation APIs, including `spawn`, pool submission, and parallel mapping, shall enforce the same Send validation.
...
For the same source, inputs, precision, and Standard Core implementation limits, Standard Core evaluation shall be deterministic except where explicitly marked unspecified. Filesystem ordering, network timing, wall-clock time, random values, GUI events, and task scheduling are hosted nondeterminism.

The reference implementation limits exact integer magnitude to 8,000,000 bits and absolute exponent to 1,000,000. Exceeding those values produces a controlled resource error rather than host memory exhaustion where detectable.
...
### 22.1 Diagnostic identifiers

The following categories are stable:

- `SAGA-L001`: lexical error;
- `SAGA-L002`: lexical resource limit;
- `SAGA-P001`: syntax/source-unit error;
- `SAGA-P002`: syntax/source-unit resource limit;
- `SAGA-T001`: static type error;
- `SAGA-T002`: type-analysis resource limit;
- `SAGA-R001`: runtime language error;
- `SAGA-R002`: runtime resource limit;
- `SAGA-I001`: implementation defect or unexpected host failure.

A diagnostic shall include code, message, source name, line, and column. JSON diagnostic schema 1 is normative for machine-readable CLI output.

### 22.2 Exit status profile
...
The CLI shall not print a host traceback unless debug mode is explicitly requested.

## 24 Implementation-defined and unspecified behavior

The following are implementation-defined and shall be reported:

- maximum source, token, AST, recursion, step, collection, and exact-number sizes above normative minima;
- decimal exponent range and trap configuration;
...
The normative EBNF is distributed as `spec/saga-0.7.ebnf` and its SHA-256 is recorded in the release checksum manifest.

## Annex B (normative) Conformance suite mapping

Every normative semantic category shall have at least one positive or negative test in the conformance manifest. Each test records an identifier, clause, mode, input hash, expected output or diagnostic code, and supported profile.
