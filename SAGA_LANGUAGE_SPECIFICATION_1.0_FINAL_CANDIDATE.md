# Saga programming language — Language Specification 1.0 Final Candidate

**Document status:** Release Candidate 2 technical specification for public implementation, independent conformance review, and possible future standards submission.  
**Document identifier:** SAGA-WD-1.0-2026-08-10  
**Normative language version:** 1.0  
**Reference qualification implementation:** Saga 0.26.2  

This document is the project Final Candidate submitted for independent technical review. It is not an ISO or IEC publication and does not imply approval, registration, certification, or endorsement by ISO, IEC, JTC 1, or SC 22.

## Foreword

Saga is designed to make introductory programming concise while retaining static type checking, exact arithmetic, explicit external capabilities, deterministic core semantics, and a path to independent implementation. The key words **shall**, **shall not**, **should**, **should not**, and **may** indicate normative requirements, recommendations, and permissions in this specification.
...
An extension shall not silently change the meaning of a conforming Saga 1.0 program. Extension syntax shall be rejected unless explicitly enabled outside the Standard Core profile.
...
The normative grammar is `spec/saga-1.0.ebnf`. When the prose and grammar appear to conflict, the prose requirements and published defect resolutions take precedence until the next published defect correction or edition.
...
Dependencies are processed before the including unit. Each canonical source path is included once. In Language Edition 1.0, included declarations share one compilation namespace; source inclusion does not create a namespace alias.
...
The `%` operator is defined only for `int` operands. For `a % b`, `b` shall be nonzero, the quotient `q` is integer division truncated toward zero, and the result is `a - q*b`. Therefore the remainder has the sign of `a` when nonzero; for example, `-7 % 3 == -1` and `7 % -3 == 1`.
...
Generic parameters are invariant in Saga 1.0. Specialization shall not change observable Standard Core behavior.
...
Standard Core diagnostic IDs include at least `SAGA-L101` through `SAGA-L106`, `SAGA-P101` through `SAGA-P102`, `SAGA-T101` through `SAGA-T112`, and `SAGA-R101` through `SAGA-R105` as listed in `spec/diagnostics-1.0.json`. The machine-readable diagnostic envelope is defined by `spec/diagnostic-schema-2.json`.
...
Saga Language Edition 1.0 freezes identifier membership to the vendored Unicode 15.1.0 XID profile so a future host Unicode upgrade cannot silently change source acceptance. The latest Unicode Standard at the time of this Final Candidate may be newer; updating the normative identifier edition requires an explicit Saga language-edition change, regenerated tables in every conforming implementation, and conformance tests.
...
`spec/SAGA_SELF_HOSTING_PROFILE_1.0_FINAL_CANDIDATE.md`. A conforming implementation claiming this optional profile uses
...
`docs/design/STABILITY_CONTRACT_1.0_FINAL_CANDIDATE.md`. New projects created by a 1.0-conforming
...
## 31 Records, enumerations, matching and typed results

### 31.1 Records

A `record` defines a named value type with declared fields. Record equality is structural by record type and field values. Records are not class identities and do not expose mutable identity semantics.

### 31.2 Enumerations

An `enum` defines a closed set of named variants. Enum variants are values of the enumeration type.

### 31.3 `match`

`match` selects a case by exact value or enum variant. When a `match` over an enum has no `default`, the compiler shall prove that all variants are covered; otherwise it shall report detailed diagnostic `SAGA-T112` or an equivalent structured diagnostic carrying the same stable ID. No case may be selected through text formatting or translated diagnostic text.

### 31.4 `result[T,E]`

A typed operation that represents explicit success or failure may use `result[T,E]`. `ok(v)` constructs the success branch and `err(e)` constructs the failure branch. Standard helpers include branch predicates and typed unwrap/default operations. A result crossing an isolated-task boundary is Send only when its contained branch value is Send.

## 32 Interpolated text and tests

Interpolated text uses `$"... ${expression} ..."`. Embedded expressions are evaluated left-to-right and converted using the Standard Core text representation; interpolation does not bypass static type checking or private-member rules.

A declaration `test "name" { ... }` defines a named standard test. The `saga test` tool executes discovered test declarations in a deterministic source order and reports failures through stable diagnostics/exit status rather than localized text matching.

## 33 Native Hosted profile

A Saga Native Hosted implementation may provide the standard modules `io`, `json`, `time`, `math`, `random`, `crypto`, `net`, `http`, `db`, `process`, `regex`, `game`, `task`, `sys`, and `compiler`. A conformance statement shall list the exact implemented API surface.

Native Hosted does not imply ambient authority. Importing a hosted module is an explicit source-level declaration of required capability category; a deployment may further deny or restrict that capability. HTTP clients shall document proxy and redirect policy. Process execution shall not invoke a command shell unless an explicitly separate non-standard API requests one.

The dependency-free terminal `game` baseline remains available. Saga Game Profile 1.0 RC1 additionally defines an optional Portable Game Profile (canonical RGBA8 framebuffer, PNG/JPEG textures, animation, camera, tilemaps, particles, AABB 2D physics, PCM WAV decoding and asset management) and an optional Desktop Game Profile (native windows, non-blocking keyboard/mouse/controller state, audio playback, renderer presentation and programmable shaders). These profiles are not Standard Core requirements. Their normative behavior is backend-neutral; SDL2/OpenGL is a reference Native backend rather than a language requirement.

## 34 Distribution, packages and self-hosting

### 34.1 Native independence

A distribution claiming **Saga Native 1.0** shall execute `run`, `check`, `test`, `fmt`, `lint`, `lock`, `verify`, `pack`, `build`, `conformance`, and `info` without requiring another programming-language runtime or compiler to be installed. Operating-system kernels, standard system calls, trust stores, device drivers, and vendor services are host facilities rather than programming-language runtime dependencies.

### 34.2 Lightweight split

The developer CLI and standalone execution runtime may be separate Saga distribution artifacts. A standalone application shall not be required to include registry, LSP, debugger, formatter, or other development-only machinery. Verified bundle source may execute directly from memory.

### 34.3 Signed packages

A signed package shall bind its canonical package SHA-256 to an Ed25519 publisher signature and public-key fingerprint. Signature validity alone does not establish trust. Installation under the standard registry profile shall require an explicit trusted-publisher decision or an equivalent deployment trust policy.

### 34.4 Fixed-point self-host compiler

A distribution claiming the Saga fixed-point self-host compiler shall publish the Saga compiler source and a bootstrap provenance statement. The release validation procedure shall generate Stage1, Stage2, and Stage3 compilers and demonstrate byte-identical Stage2/Stage3 output from the same canonical compiler source and runtime seed. Bootstrap provenance is distinct from normal runtime dependency.

## 35 Build targets and optimization

The normative language semantics are independent of code-generation target. The reference Saga Native implementation defines a deterministic standalone native runtime-AOT profile and a direct scalar WebAssembly profile. The scalar WebAssembly profile is a strict subset and shall reject unsupported Standard Core constructs rather than change their meaning.

Optimization shall occur only after required static checking. Current normative-safety requirements permit constant folding, dead unreachable branch elimination, and deterministic build caching when those transformations preserve observable Standard Core behavior and diagnostics required before optimization.

## 36 Internationalization and diagnostic stability

Source processing is independent of the user's UI language. Diagnostic IDs, ranges, categories, structured metadata, and exit status are the interoperability contract; localized human text is presentation only. English is the required fallback. Implementations may provide additional catalogs and shall fall back without changing the underlying diagnostic ID when a translation is absent.

The Language Edition 1.0 identifier table remains frozen at Unicode 15.1 even if a later Unicode edition exists. This is intentional reproducibility, not a claim that Unicode 15.1 is the newest Unicode release. A later Saga edition may update the vendored table through the compatibility process.

## 37 Finalization and review status

Language Edition 1.0 Final Candidate freezes the intended Standard Core semantics for independent review. Project publication as **Saga Language Specification 1.0 Final** requires a signed independent review attestation over the exact proposed-final bytes and closure of all normative-blocking review findings. Final publication does **not** by itself claim ISO/IEC approval, international adoption, third-party certification, or target-hardware verification. Any post-publication semantic correction shall use the defect-resolution and compatibility process and receive corresponding conformance tests.
...
This specification and its implementation artifacts are intended to support review and a possible New Work Item Proposal. Formal international-standard status requires submission by an eligible proposer, a nominated project leader, committee approval, expert participation, consensus development, ballots, and publication through the responsible standards organizations.

## Annex E (informative) Standards-development references

The project uses ISO/IEC JTC 1/SC 22 as the intended standards venue. Market relevance, eligible proposal sponsorship, Project Leader nomination, expert participation, committee consensus and ballots are external process requirements and are not satisfied merely by conformance of an implementation. The security review plan also tracks the published ISO/IEC 24772-1:2024 language-independent vulnerability catalogue at a high level; this project specification does not claim certification or conformance to that separate standard.


## Annex N (normative) — Native independent distribution

An implementation claiming the **Saga Native Distribution** profile shall also satisfy `spec/SAGA_NATIVE_DISTRIBUTION_PROFILE_1.0_FINAL_CANDIDATE.md`. In particular, the normal compiler/runtime commands shall not require another programming-language runtime or compiler toolchain to be installed on the end-user system. Bootstrap provenance is not an end-user runtime dependency and shall be disclosed separately.
