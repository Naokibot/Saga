...
3. **Keyword compatibility risk:** making common words hard keywords would break older source using names such as `await` or `move`. The 0.45 parser treats the new words contextually.
4. **Trailing-closure ambiguity in `using`:** the initializer parser could consume the resource-scope `{` as a trailing closure. Header parsing now preserves the block delimiter.
