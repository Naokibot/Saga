# Saga programming language — Language Specification 1.0 RC2
...
**Normative language version:** 1.0 RC2  
**Primary native implementation version:** 0.25.0  
...
The normative grammar is `spec/saga-1.0.ebnf`. When the prose and grammar appear to conflict, the prose requirements and published defect resolutions take precedence until the next corrected draft.
...
The normative language semantics are independent of code-generation target. Saga Native 0.25.0 defines a deterministic standalone native runtime-AOT profile and a direct scalar WebAssembly profile. The scalar WebAssembly profile is a strict subset and shall reject unsupported Standard Core constructs rather than change their meaning.
...
## 37 Release-candidate status

Language Edition 1.0 RC2 freezes the intended Standard Core semantics for independent review. The RC label does **not** claim ISO/IEC approval, international adoption, third-party certification, or target-hardware verification that has not actually occurred. Any change before 1.0 Final that alters a conforming program's meaning shall be documented as a compatibility-affecting defect resolution and receive corresponding conformance tests.
